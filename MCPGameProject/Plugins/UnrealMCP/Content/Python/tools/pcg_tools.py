...
def register_pcg_tools(mcp: FastMCP):
    ...
    def scatter_mesh(
        ...
        asset_path: str,
        actor_name: str,
        actor_location: list[float] = [504.0 * 2, 504.0 * 2],
        actor_box_extents: list[float] = [504.0 * 4, 504.0 * 4],
        ...
        Args:
            ...
            points_per_sq_meter: The point density for the PCG surface sampler.
                Recommended between 0.1 (very sparse) to 1.0 (very dense)
                (default: 10.0).
        ...
        if not mesh_assets:
            return {
                "success": False,
                "error": f"No valid mesh assets found in {asset_path}",
            }

        # Load the BP_PCGScatter Blueprint
        ...
        box_component.set_box_extent(
            unreal.Vector(actor_box_extents[0], actor_box_extents[1], 504 * 4)
        )
        ...
        # Return the actor's properties as JSON
    ...
    #     actor.set_editor_property("PointDensity", points_per_sq_meter)
    #     min_scale = min_scale / scale * 40.0
    #     actor.set_editor_property(
    #         "MinScale", unreal.Vector(min_scale, min_scale, min_scale)
    #     )
    #     max_scale = max_scale / scale * 40.0
    #     actor.set_editor_property(
    #         "MaxScale", unreal.Vector(max_scale, max_scale, max_scale)
    #     )
    #     actor.set_editor_property("MinRotation", unreal.Rotator(0.0, 0.0, min_rotation))
